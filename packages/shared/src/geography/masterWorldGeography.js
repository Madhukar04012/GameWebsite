...
/* ── 3. Subregions ── */
export const MASTER_SUBREGIONS = {
...
export const MASTER_TRAVEL_CORRIDORS = [
    {
 ...
 */
export function getRegionWeightsAt(x, z) {
 ...
 */
export function getDominantRegionAt(x, z) {
    const weights = getRegionWeightsAt(x, z);
    let bestId = "capital_kingdom";
    let maxW = -1;
 ...
 */
export function getDominantSubregionAt(x, z, dominantRegionId) {
    let nearestSub = MASTER_SUBREGIONS.city_plateau;
    let minDist = Infinity;
    for (const sub of Object.values(MASTER_SUBREGIONS)) {
        if (sub.regionId !== dominantRegionId)
            continue;
 ...
 */
export function getMacroLandformElevation(x, z) {
    const distCity = Math.sqrt(x * x + z * z);
    if (distCity <= 50)
        return 0.0; // Defensible flat city plateau
    const weights = getRegionWeightsAt(x, z);
    // Regional baseline elevation
    const baseElev = weights.frostpeak_ridge * MASTER_REGIONS.frostpeak_ridge.baseElevation +
 ...
 */
export function sampleGeography(x, z) {
    ...
    let elevationZone = "river_valley";
    if (macroElev < -0.5)
        elevationZone = "lowland_marsh";
    else if (macroElev <= 6.0)
        elevationZone = "river_valley";
    else if (macroElev <= 16.0)
        elevationZone = "plateau";
    else if (macroElev <= 26.0)
        elevationZone = "highland";
    else
        elevationZone = "alpine_crest";
    ...
    const blendedTemp = weights.capital_kingdom * MASTER_REGIONS.capital_kingdom.temperature +
    ...
    const blendedMoisture = weights.capital_kingdom * MASTER_REGIONS.capital_kingdom.moisture +
...
//# sourceMappingURL=masterWorldGeography.js.map