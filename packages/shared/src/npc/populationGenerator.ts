import { CITY_LAYOUT, DistrictDef } from "../constants";
import { NPCData, NPCScheduleBlock } from "../types/npc";
import { PRNG, generateDeterministicName } from "./nameGenerator";
import { PROFESSION_REGISTRY, ProfessionDef } from "./professions";

// Capacity definition conceptual categories
const DISTRICT_CAPACITY: Record<string, number> = {
  residential: 300,
  market: 200,
  harbor: 150,
  guild_hall: 100,
...
function generateSchedule(prng: PRNG, prof: ProfessionDef, home: {x:number, z:number}, work: {x:number, z:number}): NPCScheduleBlock[] {
...
export function generateCapitalPopulation(seed: string): NPCData[] {
  ...
  for (const district of CITY_LAYOUT) {
    ...
    if (validProfs.length === 0) continue;
    ...
    if (buildings.length === 0) {
      ...
      buildings.push({ x: district.center.x, z: district.center.z, w: 5, d: 5, h: 5, label: "Generic" } as any);
    }
