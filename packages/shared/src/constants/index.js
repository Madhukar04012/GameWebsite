...
export const ROAD_WIDTH = { main: 8, plaza: 12, district: 6, dirt: 4 };
export const ROADS = [
...
export const VEGETATION_PATCHES = [
...
export function biomeAt(x, z) {
    for (const def of BIOME_DEFS)
        if (x >= def.bounds.minX && x <= def.bounds.maxX && z >= def.bounds.minZ && z <= def.bounds.maxZ)
            return def.id;
    return "plains";
}
