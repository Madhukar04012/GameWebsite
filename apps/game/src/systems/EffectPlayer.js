...
export function useEffects() {
    ...
    const spawn = useCallback((kind, x, y, z) => {
 ...
 */
export function EffectPlayer({ effectQueue }) {
    ...
    return (_jsx("group", { children: Object.entries(BUDGET)
            .filter(([_, count]) => count > 2)
            .map(([kind]) => {
            const kindKey = kind;
            return (_jsx(EffectBurst, { kind: kindKey, effects: effects.filter((e) => e.kind === kindKey) }, kindKey));
        }) }));
}
function EffectBurst({ kind, effects }) {
    ...
    if (effects.length === 0 && spawnsRef.current.size === 0)
        return null;
    return (_jsxs("points", { ref: meshRef, children: [_jsxs("bufferGeometry", { children: [_jsx("bufferAttribute", { attach: "attributes-position", args: [geo.pos, 3], count: count, array: geo.pos, itemSize: 3 }), _jsx("bufferAttribute", { attach: "attributes-velocity", args: [geo.vel, 3], count: count, array: geo.vel, itemSize: 3 })] }), _jsx("shaderMaterial", { transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, uniforms: uniforms, vertexShader: `
          uniform float uTime;
          uniform float uSize;
