...
import { computeCameraPosition, createDefaultCamera, smoothCameraTowards, clampDistance, resolveCameraCollision, heightAt, } from "@legend/engine";
import { useDebugStore } from "../store/debugStore";
const MOUSE_SENSITIVITY = 0.003;
const SMOOTH_RATE = 8; // camera-systems skill: 5=floaty, 12=snappy. 8 is balanced.
export function ThirdPersonCamera({ playerRef }) {
    ...
    useEffect(() => {
        function onMouseMove(e) {
            ...
            target.current.pitch = Math.max(-0.8, Math.min(1.2, target.current.pitch - e.movementY * MOUSE_SENSITIVITY));
        }
    ...
    useFrame((_, delta) => {
        ...
        const smoothed = smoothCameraTowards(current.current, target.current, SMOOTH_RATE, delta);
