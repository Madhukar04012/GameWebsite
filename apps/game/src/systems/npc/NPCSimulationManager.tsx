    ...
    for (const [id, npc] of mutableNPCs.entries()) {
      ...
      if (npc.state.targetDestination) {
        ...
        if (distToTarget < 1.0) {
          ...
          if (npc.state.tier === 0) {
            // Tier 0: teleport or move very fast if far (macro sim)
            // Just move them directly at a fast simulated pace to keep them where they should be
            const moveDist = npc.state.velocity * delta * (isAccelerated ? 60 : 1); 
          ...
          } else if (npc.state.tier === 1) {
            // Tier 1: Smooth continuous movement
            const moveDist = npc.state.velocity * delta;
            if (moveDist >= distToTarget) {
              npc.state.position.x = tx;
              npc.state.position.z = tz;
            } else {
