...
function detectTier(): QualityTier {
  ...
  /* devicePixelRatio */
  if (dpr >= 2) { ultra++; high++; }
  else if (dpr >= 1.5) { high++; medium++; }
  else if (dpr >= 1) { medium++; }
  else low++;

  /* hardwareConcurrency */
  if (cores >= 12) { ultra++; high++; }
  else if (cores >= 8) { high++; medium++; }
  else if (cores >= 4) { medium++; low++; }
  else low++;

  /* maxTextureSize */
  if (maxTexSize >= 16384) { ultra++; high++; }
  else if (maxTexSize >= 8192) { high++; medium++; }
  else if (maxTexSize >= 4096) { medium++; }
  else low++;

  const scores: [QualityTier, number][] = [
    ["low", low],
    ["medium", medium],
    ["high", high],
    ["ultra", ultra],
...
type Config = Omit<GraphicsState, "quality" | "setQuality">;

const TIER: Record<QualityTier, Config> = {
  low: {
    dpr: [0.75, 1],
    shadowMapSize: 512,
    antialias: false,
...
function buildState(tier: QualityTier) {
...
export const useGraphicsStore = create<GraphicsState>((set) => ({
  ...
  setQuality: (tier: QualityTier) => set(buildState(tier)),
}));
