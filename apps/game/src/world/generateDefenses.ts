import { CITY_BOUNDS, WORLD_BOUNDS, ROADS } from "@legend/shared";
...
export function generateDefenses(): DefenseItem[] {
  ...
  const gateZones: { x: number; z: number; radius: number }[] = [];

  const addTower = (x: number, z: number, h: number, rotY: number, kind: "tower_round" | "tower_square", variant = "") => {
  ...
  const addGate = (x: number, z: number, dir: "N" | "S" | "E" | "W", isMain = false) => {
    const y = heightAt(x, z);
  ...
  // 3. Build Walls
  const buildWallSegment = (sx: number, sz: number, ex: number, ez: number) => {
    const dx = ex - sx;
    const dz = ez - sz;
    ...
    for (let i = 0; i < numSegs; i++) {
      ...
      for (const gz of gateZones) {
        if (Math.hypot(cx - gz.x, cz - gz.z) < gz.radius) skip = true;
      }
      // Corner tower gaps
      if (Math.hypot(cx - minX, cz - minZ) < 3) skip = true;
      if (Math.hypot(cx - maxX, cz - minZ) < 3) skip = true;
      if (Math.hypot(cx - minX, cz - maxZ) < 3) skip = true;
      if (Math.hypot(cx - maxX, cz - maxZ) < 3) skip = true;
