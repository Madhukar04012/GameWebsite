...
function GrassField() {
  ...
  const grassTufts = useMemo(() => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (Math.abs(x - riverX) < 10) continue;
      // 3. Skip south road
      if (Math.abs(x) < 5 && z > -80 && z < -40) continue;
  ...
  return (
    <group>
      {grassTufts.map((s, i) => {
        ...
        return (
          <group key={`gtuft-${i}`} position={[s.x, y, s.z]} rotation={[0, s.rot, 0]} scale={[s.s, s.s, s.s]}>
            {/* 3-way crossed grass blade planes */}
            <mesh position={[0, 0.4, 0]} material={grassMat}>
              <planeGeometry args={[0.9, 0.8]} />
            </mesh>
            <mesh position={[0, 0.4, 0]} rotation={[0, Math.PI / 3, 0]} material={grassMat}>
              <planeGeometry args={[0.9, 0.8]} />
            </mesh>
            <mesh position={[0, 0.4, 0]} rotation={[0, (2 * Math.PI) / 3, 0]} material={grassMat}>
              <planeGeometry args={[0.9, 0.8]} />
            </mesh>
          </group>
        );
  ...
  return (
    <group>
      {floraNodes.map((f, i) => {
        ...
        return (
          <group key={`flora-${i}`} position={[f.x, y, f.z]}>
            {/* Green Stem */}
            <mesh position={[0, 0.35, 0]} material={stemMat}>
            ...
            {f.type === "sunblossom" && (
              <>
                <mesh position={[0, 0.75, 0]} material={sunblossomMat}>
                  <sphereGeometry args={[0.22, 8, 8]} />
                </mesh>
                <Sparkles count={8} scale={[1.5, 1.5, 1.5]} size={3.5} speed={0.4} color="#ffb703" />
              </>
            )}

            {f.type === "windchime" && (
              <>
                <mesh position={[0, 0.72, 0]} rotation={[Math.PI, 0, 0]} material={windchimeMat}>
                  <coneGeometry args={[0.25, 0.35, 6]} />
                </mesh>
                <Sparkles count={8} scale={[1.5, 1.5, 1.5]} size={3.5} speed={0.5} color="#48cae4" />
              </>
            )}

            {f.type === "dandelion" && (
              <>
                <mesh position={[0, 0.75, 0]} material={dandelionPuffMat}>
                  <sphereGeometry args={[0.28, 12, 12]} />
                </mesh>
                {/* Floating Dandelion Fluff */}
                <Sparkles count={14} scale={[3, 4, 3]} position={[0, 1.5, 0]} size={2} speed={0.6} color="#ffffff" />
              </>
            )}

            {f.type === "starlight" && (
              <>
                <mesh position={[0, 0.4, 0]} material={sunblossomMat}>
                  <cylinderGeometry args={[0.08, 0.12, 0.4, 6]} />
                </mesh>
                <mesh position={[0, 0.65, 0]} material={windchimeMat}>
                  <sphereGeometry args={[0.26, 8, 6]} />
                </mesh>
                <Sparkles count={8} scale={[1.5, 1.5, 1.5]} size={3} speed={0.3} color="#b5179e" />
              </>
            )}
          </group>
        );
