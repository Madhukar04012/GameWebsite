...
function getPointAlongLine(from, to, t) {
    return {
        x: from.x + (to.x - from.x) * t,
        z: from.z + (to.z - from.z) * t,
    };
}
export function generateCityDressing() {
    const dressing = {
        barrels: [],
        crates: [],
        streetLamps: [],
    ...
    ROADS.forEach((road, i) => {
        ...
        for (let dist = spacing / 2; dist < length; dist += spacing) {
            ...
            if (road.type === "main" || road.type === "plaza") {
                ...
                if (rand(dist * i) > 0.5) {
                    dressing.planters.push({ x: pt.x + nx * (offset + 1), z: pt.z + nz * (offset + 1), rot: angle }, { x: pt.x - nx * (offset + 1), z: pt.z - nz * (offset + 1), rot: angle });
                }
                if (rand(dist * i + 1) > 0.7) {
                    dressing.benches.push({ x: pt.x + nx * (offset + 0.5), z: pt.z + nz * (offset + 0.5), rot: angle - Math.PI / 2 }, { x: pt.x - nx * (offset + 0.5), z: pt.z - nz * (offset + 0.5), rot: angle + Math.PI / 2 });
                }
            }
            else if (road.type === "district") {
                dressing.streetLamps.push(leftSpot, rightSpot);
                if (rand(dist * i) > 0.8) {
                    dressing.benches.push({ x: pt.x + nx * offset, z: pt.z + nz * offset, rot: angle - Math.PI / 2 });
                }
            }
            else {
                // Dirt / Alleys
                if (rand(dist * i) > 0.7)
                    dressing.barrels.push(leftSpot);
                if (rand(dist * i + 1) > 0.7)
                    dressing.crates.push(rightSpot);
            }
        }
    });
    ...
    CITY_LAYOUT.forEach(district => {
        ...
        district.blocks?.forEach((block) => {
            ...
            layout.props.forEach((prop, i) => {
                if (prop.type === "barrel")
                    dressing.barrels.push({ x: prop.x, z: prop.z, rot: rand(block.seed + i) * Math.PI * 2 });
                else if (prop.type === "crate")
                    dressing.crates.push({ x: prop.x, z: prop.z, rot: rand(block.seed + i) * Math.PI * 2 });
            });
        });
    });
...
function processBuildingDressing(bldg, district, seed, dressing) {
    const rx = rand(seed);
    const frontX = bldg.x;
    ...
    if (bldg.family === "commercial") {
        if (rx > 0.4)
            dressing.marketStalls.push({ x: frontX, z: frontZ, rot: frontRot });
        if (rx > 0.8)
            dressing.merchantCarts.push({ x: frontX + 2, z: frontZ + 1, rot: frontRot + 0.2 });
        if (rx > 0.6) {
        ...
        if (rx > 0.5)
            dressing.anvils.push({ x: frontX - 1.5, z: frontZ, rot: frontRot });
        if (rx > 0.3)
            dressing.woodPiles.push({ x: frontX + 1.5, z: frontZ, rot: frontRot });
        if (rx > 0.7)
            dressing.weaponRacks.push({ x: frontX, z: frontZ + 1, rot: frontRot });
    }
        ...
        if (rx > 0.7)
            dressing.statues.push({ x: frontX, z: frontZ + 2, rot: frontRot });
        if (rx > 0.5)
            dressing.hedges.push({ x: frontX - 2.5, z: frontZ, rot: frontRot });
    }
    else { // residential
        if (rx > 0.5)
            dressing.flowerBoxes.push({ x: frontX, z: frontZ - 0.5, rot: frontRot });
        if (rx > 0.7)
            dressing.laundry.push({ x: bldg.x - bldg.w / 2 - 0.5, z: bldg.z, rot: Math.PI / 2 });
        if (rx > 0.8)
            dressing.wishingWells.push({ x: bldg.x + bldg.w / 2 + 1, z: bldg.z, rot: 0 });
        if (rx > 0.6)
            dressing.woodPiles.push({ x: frontX + 1, z: frontZ, rot: frontRot });
    }
}
//# sourceMappingURL=generateCityDressing.js.map